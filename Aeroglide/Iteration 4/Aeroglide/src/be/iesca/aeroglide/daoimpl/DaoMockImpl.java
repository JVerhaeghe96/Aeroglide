...
public class DaoMockImpl implements PiloteDao, VolDao {
	...
	private Map<Integer, Vol> mapVols;
	...
	*/
	...
	public boolean enregistrerVol(Vol vol) {
		try{
			...
			this.mapVols.put(currentIdVol, vol);
			vol.setId(currentIdVol);
			currentIdVol++;
	...
	public List<Vol> listerVols(Date date) {
		Collection<Vol> liste = this.mapVols.values();
		liste = liste.stream().filter(vol -> vol.getDate().equals(date)).collect(Collectors.toList());
		return new ArrayList<>(liste);
	}
	...
	public List<Date> listerDates() {
		...
		List<Date> finalListeDates = listeDates;
		liste.forEach(vol-> finalListeDates.add(vol.getDate()));
		listeDates = finalListeDates;
		listeDates = listeDates.stream().distinct().collect(Collectors.toList());
